let API_URL = 'https://api-sandbox.nimiqoasis.com/v1';
var OasisAsset;
(function (OasisAsset) {
    OasisAsset["EUR"] = "eur";
    OasisAsset["CRC"] = "crc";
...
export async function createHtlc(contract, tokens) {
    ...
    const htlc = await api('/htlc', 'POST', contract, headers);
    return convertHtlc(htlc);
}
export async function getHtlc(id) {
    const htlc = await api(`/htlc/${id}`, 'GET');
    return convertHtlc(htlc);
}
export async function settleHtlc(id, secret, settlementJWS, tokens) {
    if (secret.length === 64) {
...
export async function sandboxMockClearHtlc(id) {
    if (!API_URL)
        throw new Error('API URL not set, call init() first');
...
export async function exchangeAuthorizationToken(token) {
    const response = await api('/auth', 'POST', undefined, {
        'Authorization': `Bearer ${token}`,
    });
    return response.token;
}
function convertHtlc(htlc) {
    const contract = Object.assign(Object.assign({ id: htlc.id, status: htlc.status, asset: htlc.asset.toUpperCase(), amount: coinsToUnits(htlc.asset, htlc.amount), fee: coinsToUnits(htlc.asset, htlc.fee, true), beneficiary: htlc.beneficiary.kty === KeyType.ELLIPTIC_CURVE ? Object.assign(Object.assign({}, htlc.beneficiary), { crv: htlc.beneficiary.crv.toLowerCase(), x: base64ToHex(htlc.beneficiary.x), y: base64ToHex(htlc.beneficiary.y) }) : Object.assign(Object.assign({}, htlc.beneficiary), { crv: htlc.beneficiary.crv.toLowerCase(), x: base64ToHex(htlc.beneficiary.x) }), hash: Object.assign(Object.assign({}, htlc.hash), { value: base64ToHex(htlc.hash.value) }), preimage: Object.assign(Object.assign({}, htlc.preimage), ('value' in htlc.preimage ? {
